...
const Cart = ({ show, onClose }) => {
  ...
  const purchaseItemHandler=(title)=>{
    setCartItems(cartItems.filter(item=>item.title!=title));
  }
  ...
  return (
    <div style={{ position: 'fixed', top: '50px', right: '10px', backgroundColor: 'white', border: '1px solid #ccc', padding: '20px', zIndex: 1000, boxShadow: '0 4px 8px rgba(0,0,0,0.2)' }}>
          ...
          {cartItems.map((item, index) => (
            <tr key={index}>
              ...
              <td>
                ...
                <input type="number" defaultValue={item.quantity} className='forn-control form-control-sm me-2' style={{ width: '40px', marginRight: '10px' }} />
      ...
      <Button variant="info" className="mt-3 text-white d-block mx-auto" onClick={()=> purchaseItemHandler(item.title)}>PURCHASE</Button>
